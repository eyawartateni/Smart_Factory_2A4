...
{
QSqlQuery query;//query c'est une requete objet de type sql query
...
query.bindValue(":marque",marque);//bind value pour se proteger du probleme de faille de securite de sql injection
...
query.bindValue(":options",options);
    ...
    return query.exec();//: Envoie la requête pour l’exécuter
}
...
QSqlQueryModel *voiture::recherche(QString REFERENCE,QString MARQUE,QString MODELE)
{
 ...
 QSqlQueryModel *model = new QSqlQueryModel();
int count =0;
// prepare:: Prendra la requête en paramètre pour la préparer à l’exécution
